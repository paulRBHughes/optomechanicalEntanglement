...
gfig, gax = plt.subplots(2, 2, figsize=sz)
# plt.tight_layout()
plt.subplots_adjust(wspace=0.01,hspace=0.01)
# tfig, tax = plt.subplots(2, 2, figsize=sz)
# plt.subplots_adjust(wspace=0.01,hspace=0.01)
positions = np.array([[0, 0], [0, 1], [1, 0], [1, 1], [2, 0], [2, 1]])
levels = np.linspace(0, 2.4, 8)  # this is the gees range
...
for i, zeta in enumerate(zetas):
    ...
    G = np.load(f'antiCVs/anticorrs{zeta}.npy')
    pos = positions[i]
...
gbar = gfig.colorbar(GCF, ax=gax).set_label(label=r"$\log_{10} (2\bar n_{th} + 1)e^{2u}$", size=16)
plt.tight_layout()
gfig.supxlabel(r"$\Delta^2_{t}$")
gfig.supylabel(r"$n_m^b$")
gfig.savefig(f"antiCVs/anticor.pdf", format='pdf', dpi=1200)
